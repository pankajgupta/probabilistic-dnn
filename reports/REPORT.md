...
### 3.6 Architecture check (exploratory) -- the frozen-swap route does not transfer to pooled CNNs

Everything above is on feedforward MLPs. As a scope probe we tried to carry the *frozen-swap* route (train conventionally, replace activations with p-neurons at inference) onto a small pooled CNN (conv channels 8,16; fc_hidden 64; 20,522 params; deterministic test accuracy 98.71% mean over 2 train seeds). This is a **2-training-seed exploratory extension**, not a full re-run of E1-E5, and it reads as a **negative result**: the route breaks, and we can name why with two identified, reproducible mechanisms.

![CNN transfer check: left, accuracy vs S under naive vs pool-fixed p-bit order with a faded MLP-mid context curve; right, per-stage noise-isolation bars localizing the residual bias to the first conv stage](../plots/fig10_cnn_transfer.png)

**Mechanism 1 -- a maxpool-over-codes bias makes even the infinite-sample limit wrong.** The naive port applies the comparator to each conv activation *before* max-pooling, so the pooled feature is a max over binary +/-1 codes. Max-pooling is nonlinear, so `E[pool(m)] != pool(E[m]) = pool(tanh(a))` -- the value the deterministic net computes. The bias does not average away: max over +/-1 codes saturates toward +1 whenever *any* unit in the window ever fires +1, so more samples do not help. Measured, the naive order sits near chance and does **not** climb with S: aggregate 14.93% at S=1 to 13.54% at S=64 (mean gain S1->S64 = -1.4 pp), against the 98.71% deterministic reference (cnn_sanity.json).

Reordering to **binarize-after-pool** (pool the real-valued activations, then apply the comparator, so downstream layers receive +/-1 codes) fixes the infinite-sample limit -- within a stage `E[m] = tanh(a)` exactly for a uniform comparator draw -- and the sampling benefit reappears: aggregate 54.05% at S=1 climbing to 87.69% at S=64 (gain +33.64 pp). But it plateaus **11.02 pp below** the deterministic reference and stays there.

**Mechanism 2 -- a residual cross-stage Jensen bias, concentrated in the weakly-saturated first conv stage.** With the order fixed, the comparator is unbiased *within* a stage, but its +/-1 output feeds a downstream nonlinear stage, and `E[downstream(m)] != downstream(E[m])`: averaging logits at the output cannot undo a nonlinearity applied to a noisy intermediate. This is a hard plateau, not slow convergence -- on a 2000-image subset (cnn_s0) accuracy is flat across a decade of extra sampling: 88.3% at S=64, 89.55% at S=512, 89.55% at S=1024, versus deterministic 98.4% (e6_cnn.json high-S plateau block). A per-stage isolation (inject p-bit noise at exactly one stage, keep the rest deterministic; S=64, 2000-image subset, cnn_s0) localizes the bias almost entirely to the first conv stage:

| stage receiving p-bit noise | accuracy |
|----|----|
| none (deterministic) | 98.40 |
| conv1 only | 89.55 |
| conv2 only | 98.10 |
| fc1 only | 98.40 |
| all stages | 89.25 |

conv1-only (89.55) reproduces nearly the entire all-stages drop (89.25), while conv2-only and fc1-only stay within 0.3 pp of deterministic. This tracks the injected per-pass variance: the first conv stage is the least saturated (mean|tanh| = 0.292, vs 0.652 at conv2 and 0.799 at fc1) and the widest (1152 comparator units vs 256 and 64), giving it by far the largest total per-pass comparator variance (980.1 vs 125.6 vs 18.8, e6_cnn.json saturation stats). Weakly-saturated units sit on the steep part of the downstream nonlinearity, where the Jensen gap between `E[downstream(m)]` and `downstream(E[m])` is largest. **We identify these two mechanisms and reproduce them; we do not claim a new positive effect.**

**H3 was not tested on the CNN.** The E3 correlation sweep (rho in {0, 0.9, 1.0}) was configured but **not run** on the CNN (`task3_ran: false`), because the ideal-noise CNN baseline never passed its sanity gate: the gap-to-deterministic gate requires <=1.5 pp and the measured gap is 11.02 pp (the multi-sample-gain gate does pass -- 33.64 pp gain vs a 3 pp floor -- so the network does sample, it just never reaches its deterministic ceiling). Degrading an ideal-noise sampling gain that itself fails the gate would not be interpretable. So **H3's transfer to CNNs remains untested -- neither confirmed nor refuted**: we do not know whether across-sample correlation would collapse a *working* CNN sampling gain, because we could not first establish one via the frozen-swap route.

**A plausible reading, flagged as interpretation not measurement.** [2] reports the multi-sample benefit for CNNs trained with sample-aware methods, and our E5 (H5) found that STE training fixes single-sample behavior on the MLP. Together these are *consistent with* noise-aware training -- rather than the frozen-swap route -- being the way to make pooled CNNs work under p-bit inference. We did **not** run STE on the CNN, so this is an interpretation connecting [2] and our H5, not a result we measured here.

**Verdict: negative (exploratory).** The frozen-swap route does not transfer to a pooled CNN: a maxpool-over-codes bias makes even the infinite-sample limit wrong under the naive order, and after the pool-order fix a cross-stage Jensen bias localized to the weakly-saturated first conv stage leaves a hard 11.02 pp plateau below the deterministic reference; because the ideal-noise CNN baseline never passed its sanity gate the E3 correlation sweep was not run, so H3's transfer to CNNs remains untested, and the H2/H3 quantitative claims stay MLP-scope.

## 4. Headline claim

**A feedforward MLP p-DNN tolerates even 1-2-bit randomness with almost no accuracy loss, but the multi-sample advantage requires independent draws: fully correlated noise (rho=1.0) erases it, high correlation (rho=0.9) defers it well past practical sample budgets, and noise-aware (STE) training mitigates but does not remove the penalty -- independence across samples, not precision, is the binding constraint.** (These are MLP-scope claims; the exploratory pooled-CNN probe in 3.6 shows the frozen-swap route does not transfer there.)

## 5. Limitations

- **Task and architecture scope.** MNIST only, and all quantitative H1-H5 results are MLP-established. We deliberately did not pursue CIFAR-10 or an RBM. The multi-sample benefit and its correlation-fragility may differ on convolutional networks or harder tasks.
- **The frozen-swap route itself breaks on pooled CNNs (3.6).** The exploratory CNN extension (2 train seeds) shows the add-noise-to-a-trained-model route does not transfer to a max-pooled CNN: a maxpool-over-codes bias makes even the infinite-sample limit wrong, and after the pool-order fix a residual cross-stage Jensen bias, concentrated in the weakly-saturated first conv stage, leaves a hard ~11 pp plateau below deterministic. Because that ideal-noise CNN baseline failed its sanity gate, the E3 correlation sweep was not run on the CNN, so H3's transfer to CNNs is untested (not refuted). All H2/H3 quantitative claims are therefore MLP-scope.
